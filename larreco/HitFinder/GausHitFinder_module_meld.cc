...
namespace hit {
  ...
  private:
    ...
    std::function<double(double, double, double, double, int, int)> getChargeFunction(bool fAreaMethod, const std::vector<double>& fAreaNormsVec);
 ...
 std::function<double(double, double, double, double, int, int)> GausHitFinder::getChargeFunction(bool fAreaMethod, const std::vector<double>& fAreaNormsVec) {
    // Return the appropriate charge function based on the area method                                                                                                                                      
    if (fAreaMethod == 0) {
        return [](double peakMean, double peakAmp, double peakWidth, double areaNorm, int low, int hi) {
          double charge = 0;
    ...
    tbb::parallel_for(
      ...
      [&](size_t& wireIter) {
		...
		chi2PerNDFValues.push_back(chi2PerNDF);		
              ...
              // #######################################################
	      int nHitsThisPulse = std::min((endT - startT) / fLongPulseWidthVec.at(plane), fLongMaxHitsVec.at(plane));
	      int longPulseWidth = (endT - startT) / nHitsThisPulse;
	      ...
	      */
              ...
              for (const auto& peakParams : peakParamsVec) {
                ...
                // ### Charge ###
                float charge =
		  getChargeFunction(fAreaMethod, fAreaNormsVec[plane]);
		...
		auto partitionPoint = std::partition(filteredHitVec.begin(), filteredHitVec.end(),
						     [largestPH, threshold](const auto& hit) {
						       return !(hit.PeakAmplitude() < 8. && hit.PeakAmplitude() / largestPH < threshold);
						     });

		// Erase the non-qualifying hits in one operation                                                                                                                                         
		filteredHitVec.erase(partitionPoint, filteredHitVec.end());
		// Resort in time order if necessary                                                                                                                                                      
		std::sort(filteredHitVec.begin(), filteredHitVec.end(),
			  [](const auto& left, const auto& right) {
